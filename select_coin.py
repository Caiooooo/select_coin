...
class CoinSelector:
    ...
    def get_all_coins(self, limit: int = 2500) -> List[Dict]:
        """
    ...
    def is_excellent_coin(self, coin: Dict) -> bool:
        """
        判断是否为优秀币种
        :param coin: 币种数据
        :return: 是否为优秀币种
    ...
    def filter_coins(self, limit: int = 2500) -> List[Dict]:
        """
        筛选优秀币种
        :param limit: 获取币种数量限制
        :return: 筛选后的币种列表，按评分排序
        """
        ...
        for coin in all_coins:
            if self.is_excellent_coin(coin):
...
def main():
    ...
    filtered_coins = selector.filter_coins(limit=2500)
